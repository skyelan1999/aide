...
> **现状核对（2026-09-25，版本 0.1.10.2 RC1）**：FR-58~FR-60 已落地；面板后续扩展为 12 分区（stats/appearance/language/model/sessions-data/permissions/account/persona/voice/accessibility/backup/about）。本文 §5 两条「非目标」**已被后续需求打破**：① 面板现已收纳「模型参数」（采样 Profile）、账户/锁屏、aide 性格、语音小秘、无障碍、配置备份、权限管理、消耗统计等分区；② 现已提供「配置备份」导出/导入（`POST /api/config/export|import`）。外观/语言偏好仍存于 `localStorage['aide.ui']`（含 `palette` blue/green、`language`），其余设置落服务端。
...
> ⚠️ 以下两条为 2026-09-21 当时的非目标，**后续已被打破**（见文首「现状核对」）：面板现已收纳模型参数、账户、性格、语音、无障碍、配置备份等分区，并提供配置导出/导入。

- ~~不做设置面板收纳模型配置（模型设置对话框保留现状）。~~ **已打破**：面板新增「模型参数」等分区；多模型列表仍在模型设置弹窗。
- 不做主题第 3/4 套方案（仍 2 套）。
- ~~不做设置云端同步 / 导出导入。~~ **已打破**：新增「配置备份」分区，支持本地导出/导入（`/api/config/export|import`）。
- 不改动 Go 后端任何接口。
