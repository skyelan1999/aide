...
侧栏最顶部永久固定一个 🤖 小秘系统会话，它就是语音秘书「小秘」的对话视图：

- **永久置顶**：永远排在普通置顶会话之前，不可归档、不可删除（菜单中不显示归档/删除）。
- **名字跟随**：会话标题 = 设置里的「语音助手名字」，改名后标题自动跟随。
- **密码进入**：点击小秘会话先弹出密码门，需输入账户密码（即锁屏密码）才能进入；验证通过后本标签页内保持解锁，锁屏后需重新验证。
- **跨会话工具**：小秘在自己的会话里可以调用四个工具——`search_sessions`（关键词搜全部会话含归档）、`get_session`（按 `#N` 或会话 ID 读取）、`follow_session`（标记跟进，有更新时提醒）、`push_to_session`（向指定会话推送备注/总结）。这些工具不对普通会话暴露。
