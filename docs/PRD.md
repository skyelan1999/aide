...
| 项 | 内容 |
| --- | --- |
| 文档版本 | v3.1（全貌重编 + 语音小秘等补充） |
| 日期 | 2026-09-25 |
| 状态 | 草稿（Draft） |
| 对应代码基线 | `internal/server` 当前工作树（workflow.go / server.go / context.go / persona.go / command.go） |
| 前序基线 | v3.0（2026-09-24 全貌重编）；v2.1（2026-09-23，FR-01~100 状态表，见[附录 A](#附录-a历史-fr-基线-fr-01100)） |
...
| SM-12 | 取消任务 | `POST /runs/{run}/cancel` 立即中断流；EventSource 断开轮询降级 | 取消后 Run 转终态、不再消耗 | P0 | 已实现 |
...
| WF-13 | md 相对路径图片加载 | 渲染时把相对 `<img src>` 改写为 aide 文件原始字节接口，正确解析相对当前文档目录的 `../`；支持 png/jpg/jpeg/gif/svg/webp；**模态预览 / 新标签页视图 / 会话消息三处都生效**；外部 http(s) 图片不受影响；加载失败不显示破图问号 | 仓库文档里相对图片（如 `docs/images/*.jpg`）正常显示；外链图片原样加载；失败静默不渲染破图 | P1 | **待修复**（当前显示 `?` 破图，见截图） |
...
| MD-08 | 推理强度选择栏 | 选择栏列出 DeepSeek 官方档位（映射真实 API 参数 `thinking` / `reasoning_effort`）+「自动」；选档后**真实转成请求参数**下发，不是摆设；设置持久化；自动路由时 Lead 可按任务复杂度选档 | 选档后请求体带对应 reasoning 字段；刷新后保留；自动模式 Lead 能选档 | P1 | **开发中**（字段名待与 DeepSeek 实际 API 核对，见 OQ-06） |
...
| AU-08 | 工具轮次上限 | 默认 60 轮，设置→权限管理可配 5–200 | 改配置后生效；到达上限停止 | P1 | 已实现 |
...
| AG-04 | 自动模式多智能体编排 | 工作流下不选阶段时注入 `autoModePrompt`：Lead 前台澄清需求 → 按实际数值选 Profile → 多次 `spawn_subagent` 分工（需求/设计/实施/验证）→ 读产出逐条面向用户核对闭环 | Lead 真正多次召唤子 Agent 并核对；不满足的补做 | P0 | **开发中** |
| AG-05 | 自动路由按数值匹配 | `profileInventoryPrompt()` 列出各 Profile 真实 temperature/top_p/max_tokens；严谨环节选低温；配置不足**暂停**并建议具体参数，不硬选 | Lead 传 profile id；缺参数时停下询问 | P1 | **开发中** |
...
| TR-10 | 思考过程折叠 | `reasoning_content`/思考过程**默认折叠**成「思考过程」条，可展开窥测；思考进行中显示"思考中…"状态；结束后可收起 | 思考内容默认不占屏、可点开看；状态正确 | P2 | 待开始 |
...
| UI-04 | 消息操作条 | 每条 AI 回复下：📋复制 / 🔄重试 / ⏵继续 / 👍好的回答 / 👎有问题（统一样式纯按钮） | 复制进剪贴板；好/坏上报 feedback | P2 | 部分实现（见 OQ-01） |
...
| 编号 | 功能点 | 业务规则 | 验收标准 | 优先级 | 状态 |
| --- | --- | --- | --- | --- | --- |
| VO-01 | 麦克风听写入口 | 麦克风按钮 + 弹出框实时听写；先用浏览器 Web Speech API 做前端识别，**预留 Whisper** 后端转写接口 | 点麦克风开始听写，弹出框实时显示转写文本 | P1 | 待开始 |
| VO-02 | 小秘作为独立 Agent | 小秘有自己的系统设定、上下文、自我分析推理能力；有独立记忆；可调用工具或 `spawn_subagent` 委派其他 Agent | 小秘能基于听到的内容做判断，而非机械转写 | P1 | 待开始 |
| VO-03 | 动态断句与直接发送 | 自动按停顿/语义边界动态断句；甄别为指令后**直接发送到当前会话**（不是填入输入框等回车） | 说完一句话即自动作为任务发出，无需手动发送 | P1 | 待开始 |
| VO-04 | 环境声与人声甄别 | 自动忽略背景视频/电视声；检测到用户在和他人闲聊时自动退下（不插话）；置信度低时不贸然发送 | 电视/会议背景音不触发发送；闲聊时小秘静默 | P1 | 待开始 |
| VO-05 | 语音记录可视化 | 语音框展示「已发送 / 已忽略」记录，逐条可见 | 能回看每段话是被发了还是被忽略 | P2 | 待开始 |
| VO-06 | 小秘设置与历史 | 设置中可自定义小秘名字；可查看小秘完整工作历史（听到了什么 / 如何分析 / 发了哪条 / 忽略了哪条 / 调用了谁），持久化、重启不丢 | 改名生效；历史面板完整、重启后仍在 | P2 | 待开始 |
| VO-07 | 模式兼容 | 发出的语音指令兼容排队/插入模式、对话模式/AI 工作流模式 | 语音发的任务与打字发的走同一条发送链路 | P1 | 待开始 |
...
| --- | --- | --- | --- | --- | --- |
| M1 会话管理 | SM-01~12 | 12 | 12 | 0 | 0 |
| M2 工作区与文件 | WF-01~13 | 13 | 12 | 0 | 1（WF-13 md 图片待修复） |
| M3 内置工具 | TL-01~17 | 17 | 17 | 0 | 0 |
| M4 模型与策略 | MD-01~08 | 8 | 7 | 1（MD-08 推理强度） | 0 |
| M5 权限与沙箱 | AU-01~08 | 8 | 8 | 0 | 0 |
| M6 记忆/性格/反馈 | MM-01~05 | 5 | 4 | 0 | 1（MM-05） |
| M7 多 Agent | AG-01~05 | 5 | 3 | 2（AG-04/05） | 0 |
| M8 工作流四阶段 | FL-01~06 | 6 | 6 | 0 | 0 |
| M9 轨迹与统计 | TR-01~10 | 10 | 8 | 0 | 2（TR-09 顺滑排查 / TR-10 思考折叠） |
| M10 部署环境 | DP-01~06 | 6 | 6 | 0 | 0 |
| M11 插件 | PL-01~04 | 4 | 4 | 0 | 0 |
| M12 外观交互 | UI-01~07 | 7 | 6 | 0 | 1（UI-04 重试路由） |
| M13 语音小秘 | VO-01~07 | 7 | 0 | 0 | 7 |
| **合计** | — | **108** | **93** | **3** | **12** |
...
| md 相对路径图片加载（三处视图生效、失败不显示破图） | WF-13 | 待修复（当前破图） |
| 消息操作条（复制/重试/继续/好/坏） | UI-04 / MM-02 | 部分实现（重试路由待补） |
| 四阶段按钮 + 四阶段后端建档 | FL-01~06 | 已实现 |
| 自动模式多智能体编排 + 自动路由（按实际参数核对/不足暂停建议） | AG-04/05 | 开发中 |
| 推理强度选择栏（thinking/reasoning_effort + 自动，真实转 API 参数） | MD-08 | 开发中 |
| SSE 顺滑性排查优化（flush/代理缓冲/前端渲染） | TR-09 | 待排查优化 |
| 思考过程默认折叠可展开 | TR-10 | 待开始 |
| 策略浮层「自动路由」文字换行修复 | UI-07 | 已修复 |
| 语音小秘（听写/真 Agent/直接发送/环境声甄别/历史/改名/模式兼容） | VO-01~07 | 待开始 |
| E2E 浏览器 UI 验收 | — | 待验证 |
| 容器重建后全流程实测 | — | 待验证 |
...
| 编号 | 问题 | 现状 | 建议 |
| --- | --- | --- | --- |
| OQ-01 | 消息「🔄重试」前端调用 `POST /sessions/{id}/runs/{run}/retry`，但 mux 中**未注册该路由** | 按钮存在、后端 404 | 补后端 retry 路由（基于原 prompt 重跑一次）或暂时禁用按钮 |
| OQ-02 | `search_text` 工具 schema 里 `required` 误写为 `"command"`，实际参数是 `query`/`path` | schema 描述与校验不一致 | 修正 required 为 `["query"]` |
| OQ-03 | `semantic_search` 是离线 TF-IDF 余弦相似度，并非向量 embedding；命名易被误解为真语义检索 | 实现与"语义"字面有差距 | 在 UI/提示中明确标注"本地词频语义、非向量检索"，或后续接 embedding |
| OQ-04 | 远程 SSH/SFTP 工作区下 `run_shell` 不自动执行（提示手动运行） | 已知边界 | 后续打通远程 exec |
| OQ-05 | MCP 来源仅登记、不实际协议调用 | 已登记入口 | 视需要实现 MCP client |
| OQ-06 | 推理强度（MD-08）后端尚无 reasoning 参数透传，设置与 API 映射待接 | 开发中 | 确认 DeepSeek 实际 reasoning 字段名与档位枚举后接线 |
| OQ-07 | 无交互式 PTY，不适合常驻服务/交互编辑器 | 已知边界 | 列入非目标，不本期实现 |
| OQ-08 | 本次多项新功能缺真实浏览器 E2E 与容器重建后全流程实测 | 待验证 | 容器重建后按 §5 流程图跑四阶段+自动模式全链路 |
