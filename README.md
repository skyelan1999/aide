...
![aide 工作台预览：会话、项目文件与 AI 任务输入](docs/images/workbench-preview.jpg)
 ...
 > **版本**：当前版本 `0.1.10.2 RC1`。本轮新增持久记忆（read/write_memory，每次会话自动注入系统提示）、模型上下文窗口预设按钮（32K/64K/128K/200K/256K/1M 一键选）、工具调用轮次默认 60（可在权限管理 5–200 调整）、md/Mermaid 渲染修复与相对路径链接内部打开、轨迹导出与调用分析视图、上下文预览彩色堆叠图、子 Agent（spawn_subagent）、失败自动重试与止损提示、Codex 风格三级沙箱、AI 工作流四阶段与自动模式、推理强度五档、语音小秘、per-tool 权限开关、配置备份、锁屏与性格持久化；此前版本已含会话管理、排队/插话、SSE 流式输出。截图来自同套界面的隔离示例环境；安装包与发布状态以 [GitHub Releases](https://github.com/skyelan1999/aide/releases) 为准。
...
| 插件 | 上传、启停、展示工具；带 handler 的工具进入调用循环；内置 draw.io 画图插件 | 仅兼容形态子集；插件是可信代码，不是安全沙箱 |
| 权限、语音与个性化 | 三级沙箱（只读/工作区可写/完全访问）、per-tool 开关、锁屏；语音小秘转写、性格持久化；配置备份导出/导入 | 沙箱约束容器权限，不是对恶意插件的安全保证；语音识别在浏览器侧完成 |
