...
func TestCompleteStreamTextDeltas(t *testing.T) {
	...
	text, calls, usage, err := completeStream(context.Background(), cfg, []Message{{Role: "user", Content: "hi"}}, ProfileParams{}, nil, nil, func(d string) { deltas = append(deltas, d) })
	if err != nil {
...
func TestCompleteStreamToolCallAssembly(t *testing.T) {
	...
	cfg := Settings{BaseURL: srv.URL, Model: "test-model"}
	text, calls, _, err := completeStream(context.Background(), cfg, []Message{}, ProfileParams{}, builtinTools[:1], nil, func(string) {})
	if err != nil {
		t.Fatalf("unexpected err: %v", err)
	}
	if len(calls) != 1 {
...
func TestCompleteStreamNonSSEFallback(t *testing.T) {
	...
	cfg := Settings{BaseURL: srv.URL, Model: "test-model"}
	text, _, _, err := completeStream(context.Background(), cfg, []Message{}, ProfileParams{}, nil, nil, func(string) {})
	if err != nil {
		t.Fatalf("unexpected err: %v", err)
	}
	if text != "fallback" {
...
func TestCompleteStreamHTTPError(t *testing.T) {
	...
	cfg := Settings{BaseURL: srv.URL, Model: "test-model"}
	_, _, _, err := completeStream(context.Background(), cfg, []Message{}, ProfileParams{}, nil, nil, func(string) {})
