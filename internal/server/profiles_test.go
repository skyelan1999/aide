...
func TestProfileParamsReachProvider(t *testing.T) {
	...
	if _, err := complete(context.Background(), Settings{BaseURL: provider.URL, Model: "deepseek-chat"}, nil, params); err != nil {
		t.Fatal(err)
	}
	...
	// reasoner：不支持参数被剔除
	if _, err := complete(context.Background(), Settings{BaseURL: provider.URL, Model: "deepseek-reasoner"}, nil, params); err != nil {
		t.Fatal(err)
	}
